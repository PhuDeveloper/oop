...
public class FormSell extends javax.swing.JPanel {
    ...
    public void fillTableTemp() {
        if (!Validate.checkEmpty(lblQuantity, txtQuantity, "Số lượng không bỏ trống")) {
            ...
            if (row == -1) {
                ...
                //So sánh nếu chọn trùng sản phẩm thì chỉ cộng số lượng
                int rowsCount = model.getRowCount();
                if (rowsCount > 0) {
                    for (int i = 0; i < rowsCount; i++) {
                        if (id == (int) tableSellTemp.getValueAt(i, 0)) {
                            model.setValueAt(quantity + (int) model.getValueAt(i, 7), i, 7);
                            break;
                        }
                        else{
                            if(i< rowsCount-1)
    ...
    InvoiceSell getInvoiceSell() {
        ...
        if (txtMoneyCustomer.getText().trim().isEmpty() == true) {
            in.setMoneyCustomer(0);
            in.setMoneyReturn(0);
        } else {
        ...
        if (!jcheckVoucher.isSelected()) {
            System.out.println("Not use vouchers");
    ...
    public void insertInvoiceSell() {
        ...
        if (count <= 0) {
            ...
            if (txtMoneyCustomer.getText().trim().isEmpty() == false && Float.valueOf(txtMoneyCustomer.getText()) - Float.valueOf(TotalBuy()) < 0) {
                MsgBox.alert(this, "Kiểm tra số tiền khách đưa đã đủ ?");
                return;
            } else {
                InvoiceSell in = getInvoiceSell();
                in.setPrice(MoneyVoucher());
                if (txtMoneyCustomer.getText().trim().isEmpty() == true) {
                    MsgBox.alert(this, "Hoá đơn chưa được thanh toán");
                ...
                MsgBox.alert(this, "Tạo 1 đơn hàng thành công");
                txtTotal.setText("");
                if (jcheckVoucher.isSelected()) {
                    Voucher v = (Voucher) cbbVoucher.getSelectedItem();
                                                        ...
                                                        .addGroup(jPanel4Layout.createSequentialGroup()
                                                                ...
                                                                .addGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                                                        ...
                                                                        .addGroup(jPanel4Layout.createSequentialGroup()
                                                                                ...
                                                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                                                .addComponent(myButton3, javax.swing.GroupLayout.PREFERRED_SIZE, 37, javax.swing.GroupLayout.PREFERRED_SIZE)))
                                                                .addGap(25, 25, 25))
                                                        .addComponent(scrollBarCustom2, javax.swing.GroupLayout.DEFAULT_SIZE, 306, Short.MAX_VALUE))))
                                .addGap(49, 49, 49))
        );
...
//        txtReturn.setText(nf.format(Float.valueOf(txtMoneyCustomer.getText()) - Float.valueOf(TotalBuy())) + " đ");
    ...
    private void txtMoneyCustomerKeyReleased(java.awt.event.KeyEvent evt) {
        // TODO add your handling code here:
        if (txtMoneyCustomer.getText().isEmpty()) {
            txtReturn.setText("");
            return;
