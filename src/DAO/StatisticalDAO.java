...
public class StatisticalDAO {
    ...
    public int getRevenueDate() {
        ResultSet rs;
        //Đức làm tham khảo
        String sql = "SELECT CAST(SUM(totalMoney) AS INT) revenue from dbo.InvoiceSell \n" 
                + "WHERE \n" 
                + "YEAR(InvoiceSell.dateCreateInvoice) = YEAR(GETDATE()) \n" 
                + "AND MONTH(InvoiceSell.dateCreateInvoice) = MONTH(GETDATE()) \n" 
                + "AND DAY(dbo.InvoiceSell.dateCreateInvoice) = DAY(GETDATE())";
