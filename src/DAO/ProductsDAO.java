...
public class ProductsDAO extends ShopDAO<Products, Integer> {
    ...
    public void insertx(Products e) {
        String sql = "INSERT INTO dbo.Products(idSupplier,idList,nameProduct,description,status)values(?,?,?,?,?)";
        jdbcHelper.update(sql,3, e.getIdList(), e.getNameProduct(), e.getDescription(), e.isStatus());
    }
